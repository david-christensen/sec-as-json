...
```bash
sec-as-json$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
sec-as-json$ sam logs -n HelloWorldFunction --stack-name sec-as-json --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sec-as-json
```
